...
def _get_client() -> chromadb.PersistentClient:
    """
    Initialize (or reuse) a persistent ChromaDB client that stores data under
    the top-level `database/chroma_data/` directory.
    """
...
def _get_case_collection():
    """
    Return the Chroma collection for the Kottayam Star case.
    """
...
def initialize_case_data() -> None:
    """
    Create and seed the `case_kottayam_star` collection with core facts
    about the stolen chip, Leo, and Dr. Tara.

    This function is safe to call multiple times; it will not duplicate
    documents if they already exist.
    """
    collection = _get_case_collection()

    # If the collection already has data, assume it is initialized.
...
def retrieve_context(query: str, character_id: str | None = None, n_results: int = 4) -> List[str]:
    """
    Given an interrogation question and optional character_id, return the most
    relevant stored facts for the LLM to use as context.

    If character_id is provided, prioritize facts for that character plus
    general case facts. Otherwise, search across all facts.
    """
    collection = _get_case_collection()

    where = None
    if character_id:
        # Include both character-specific and general facts.
    ...
    # Chroma returns documents as a list of lists (one list per query).
